...
const ensure_meta_table = (db: Database): void => {
  db.run("CREATE TABLE IF NOT EXISTS _db_meta (key TEXT PRIMARY KEY, value TEXT NOT NULL)")
}

const read_init_seeds = (db: Database): boolean => {
  const row = db
    .query<{ value: string }, [string]>("SELECT value FROM _db_meta WHERE key = ?")
    .get("init_seeds")
  return row?.value === "true"
}

const write_init_seeds = (db: Database, value: boolean): void => {
  db.query("INSERT OR REPLACE INTO _db_meta (key, value) VALUES ('init_seeds', ?)").run(
    value ? "true" : "false",
  )
}

...
const apply_version = 3

// ponytail: start from a clean db so the example is re-runnable
rmSync(db_path, { force: true })

const db = migrate_db(migrations_path, db_path, true, apply_version)

const { user_version } = db.query("PRAGMA user_version").get() as { user_version: number }
if (user_version !== apply_version) {
  throw new Error(`expected user_version ${apply_version}, got ${user_version}`)
}
...
console.log(`ok: latest migration ${apply_version} applied to ${db_path}`)
